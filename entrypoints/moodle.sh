...
function set_moodle_run_env() {
    ...
    MOODLE_DATA_BASE_DIR=${SHARED_DATA_DIR}/moodledata/${MOODLE_VERSION}/${DB_YPE}
...
function install_moodle() {
    echo "Installing Moodle for ${DB_TYPE} database"
    whereami="${PWD}"
    cd $MOODLE_DIR

    php admin/cli/install_database.php --adminpass=moodle --adminemail=moodle@example.com --fullname="Moodle Test Site (${DB_TYPE})" --shortname="MoodleTestSite-${DB_TYPE}" --agree-license > /dev/null 2>&1
